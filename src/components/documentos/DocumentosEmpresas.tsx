import { Search, Building2, ChevronRight, Plus, Filter, CalendarDays, Clock, UploadCloud, ExternalLink, Edit3, Paperclip } from 'lucide-react';
...
export function DocumentosEmpresas(props: {
  empresas: EmpresaRow[];
  ...
  empresaStats: Map<string, EntityDocStats>;
  ...
  onOpenNovaEmpresa: () => void;
  onOpenEditEmpresa: (empresa: EmpresaRow) => void;
}) {
  const {
    empresas,
    selectedEntidadeId,
  ...
  const selectedEmpresaRow =
    empresas.find((e) => e.id === selectedEntidadeId) || null;

  const selectedLabel = selectedEmpresaRow
    ? String(selectedEmpresaRow.nome || selectedEmpresaRow.razao_social || selectedEmpresaRow.id)
    : '';
      ...
      <Card className={`p-5 ${cardBase}`}>
        <div className="flex items-center justify-between gap-2">
          <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">
            Empresas ({sortedEntities.length})
          </div>

          <Button variant="secondary" onClick={onOpenNovaEmpresa}>
      ...
      <Card className={`p-5 ${cardBase}`}>
          ...
          <div>
            ...
            <div className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              {selectedEntidadeId ? (
                ...
                'Selecione uma empresa na lista ao lado para carregar os documentos.'
              )}
            </div>
          </div>
            ...
            <div className="mt-4 flex flex-col xl:flex-row gap-4 xl:items-center xl:justify-between">
              <div className="flex flex-col lg:flex-row gap-3 lg:items-center">
                <div className="relative w-full lg:w-[420px]">
                  ...
                  <input
                    type="text"
                    placeholder={`Pesquisar em ${scopeLabel('empresa').toLowerCase()}… (nome, tipo, validade, ficheiro)`}
                    value={search}
        ...
        {!selectedEntidadeId && (
          <div className="mt-10 text-center text-slate-500 dark:text-slate-400">
            Selecione uma empresa na lista ao lado para ver os documentos.
          </div>
        )}
      </Card>
    </div>
